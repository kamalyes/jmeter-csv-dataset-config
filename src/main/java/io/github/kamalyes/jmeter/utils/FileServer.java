 ...
 */
package io.github.kamalyes.jmeter.utils;
...
public class FileServer {
    ...
    private FileServer() {
        ...
        LoggerUtil.info("Default base='{}'", DEFAULT_BASE);
    }
    ...
    public synchronized void resetBase() {
        checkForOpenFiles();
        base = new File(DEFAULT_BASE);
        LoggerUtil.info("Reset base to '{}'", base);
    }
    ...
    public synchronized void setBasedir(String basedir) {
        ...
        if (basedir != null) {
            ...
            base = newBase;
            LoggerUtil.info("Set new base='{}'", base);
        }
    }
    ...
    public synchronized File getBaseDirRelative() {
        ...
        File absoluteFile = base.getAbsoluteFile();
        ArrayDeque<String> deque = new ArrayDeque<>();
        while (absoluteFile != null) {
            if (absoluteFile.equals(parent)){
                if (deque.isEmpty()){
                    break;
                }
                File rel = new File(deque.pop());
                while(!deque.isEmpty()) {
                    rel = new File(rel, deque.pop());
                }
                return rel;
            }
            deque.push(absoluteFile.getName());
    ...
    public synchronized String reserveFile(String filename, String charsetName, String alias, boolean hasHeader) {
        ...
        if (fileEntry == null) {
            ...
            if (filename.equals(alias)){
                LoggerUtil.info("Stored: {}", filename);
            } else {
                LoggerUtil.info("Stored: {} Alias: {}", filename, alias);
            }
    ...
    public String[] csvReadLine(String line, char delimiter) throws IOException {
        ...
        CharArrayWriter buff = new CharArrayWriter(200);
        boolean push = false;
        while (index < length) {
            push = false;
            ...
            switch (state) {
                ...
                case PLAIN:
                    if (ch == QUOTING_CHAR) {
                        buff.write(ch);
                        throw new IOException("Cannot have quote-char in plain field:[" + buff.toString() + "]");
                    } else if (isDelimOrEOL(delimiter, ch)) {
                        push = true;
                ...
                case EMBEDDEDQUOTE:
                    if (ch == QUOTING_CHAR) {
                        buff.write(QUOTING_CHAR); // doubled quote => quote
                        state = ParserState.QUOTED;
                    } else if (isDelimOrEOL(delimiter, ch)) {
                        push = true;
                        state = ParserState.INITIAL;
                    } else {
                        buff.write(QUOTING_CHAR);
                        throw new IOException("Cannot have single quote-char in quoted field:[" + buff.toString() + "]");
                    }
                    break;
                default:
        ...
        if (state == ParserState.QUOTED) {
            throw new IOException("Missing trailing quote-char in quoted field:[\"" + buff.toString() + "\"]");
        }
        // Do we have some data, or a trailing empty field?
        if (buff.size() > 0 // we have some data
