package io.github.kamalyes.jmeter.config.gui;

import io.github.kamalyes.jmeter.config.CsvDataSetConfig;
import io.github.kamalyes.jmeter.utils.BrowseAction;
import io.github.kamalyes.jmeter.utils.LoggerUtil;
...
public class CsvDataSetConfigGui extends AbstractConfigGui {
    private static final long serialVersionUID = 240L;
    private static final String DISPLAY_NAME="Kamalyes CSV Data Set Config";
    ...
    public TestElement createTestElement() {
        CsvDataSetConfig element = new CsvDataSetConfig();
    ...
    public void modifyTestElement(TestElement element) {
        ...
        if(element instanceof CsvDataSetConfig){
            CsvDataSetConfig eCsvDataSetConfig = (CsvDataSetConfig) element;
            ...
            LoggerUtil.debug("{}", eCsvDataSetConfig.printAllProperties());
        }
    }
    ...
    private void init() {
        ...
        ignoreFirstLineCBox.addActionListener(e-> LoggerUtil.debug("Ignore First line in csv is set as : {}", ignoreFirstLineCBox.getSelectedItem()));
        quotedDataCBox.addActionListener(e-> LoggerUtil.debug("Quoted data in csv is set as : {}", quotedDataCBox.getSelectedItem()));
        sharingModeCBox.addActionListener(e-> LoggerUtil.debug("Sharing mode is set as : {}", sharingModeCBox.getSelectedItem()));

        selectRowCBox.addActionListener(e -> {
            LoggerUtil.debug("Selection is : {}", selectRowCBox.getSelectedItem());
        ...
        viewFileButton.addActionListener(e -> {
            try {
                ...
                if(!file.exists()){
                    int selection = JOptionPane.showConfirmDialog(new CsvDataSetConfigGui(), "File does not exist. Do you want to create it ?",
                ...
                if(file.exists()){
                    if(desktop.isSupported(Desktop.Action.EDIT)){
                        ...
                        JOptionPane.showMessageDialog(new CsvDataSetConfigGui(), "Unable to get the default editor");
                    }
                }
            } catch (FileNotFoundException fne){
                JOptionPane.showMessageDialog(new CsvDataSetConfigGui(),"Invalid File path.");
    ...
    public void configure(TestElement element) {
        ...
        if(element instanceof CsvDataSetConfig){
            CsvDataSetConfig config = (CsvDataSetConfig) element;
