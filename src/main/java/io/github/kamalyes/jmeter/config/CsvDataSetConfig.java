package io.github.kamalyes.jmeter.config;

import io.github.kamalyes.jmeter.utils.FileServer;
...
public class CsvDataSetConfig extends ConfigTestElement implements LoopIterationListener, TestStateListener, NoConfigMerge {
    ...
    public void iterationStart(LoopIterationEvent iterationEvent) {
        ...
        switch(getSelectRow().toLowerCase()){
            case "sequential":
                try{
                    ...
                    LoggerUtil.debug("Sequential line fetched : {}", line);
                    if(isQuotedData()){
            ...
            case "random":
                try{
                    ...
                    LoggerUtil.debug("Random line fetched : {}", line);
                    if(isQuotedData()){
                        lineValues = fileServer.csvReadLine(line, delimiter.charAt(0));
                    }else{
                        lineValues = JOrphanUtils.split(line, delimiter, false);
            ...
            case "unique":
                try{
                    String line = fileServer.readUnique(alias, ignoreFirstLine, getOoValue(), FileServer.getReadPos(), FileServer.getStartPos(), FileServer.getEndPos());
                    LoggerUtil.debug("Unique line fetched : {}", line);
                    if(isQuotedData()){
                        lineValues = fileServer.csvReadLine(line, delimiter.charAt(0));
                    }else{
                        lineValues = JOrphanUtils.split(line, delimiter, false);
        ...
        switch (getPropertyAsString(UPDATE_VALUE).toLowerCase()) {
            ...
            case "once":
                ...
                break;
            default:
                LoggerUtil.error("Invalid selection on Update Value");
                throw new JMeterStopThreadException("Invalid selection :" + getFilename() + " detected for CSV DataSet:"
                        + getName() + " configured to Select Row Parameter :" + getUpdateValue());
        }
    }

    private void initVars(FileServer fileServer, JMeterContext context, String delimiter) {
    ...
    private void initBlockFeatures(JMeterContext context) {
        ...
        if(isAutoAllocate()){
            blockSize = FileServer.getRowCount() / JMeterContextService.getTotalThreads();
        }else{
            blockSize = Integer.parseInt(getBlockSize());
            if(blockSize < 1){
                throw new JMeterStopThreadException("Block Size Allocation Exception :" + getBlockSize()
                        + " Please Ensure the block size is greater than 0"
                        + " Or select auto allocate feature, which is currently set to : " + isAutoAllocate());
            }
        }
        //Set Start and end position to block
        FileServer.setReadPosition(threadName, blockSize, isIgnoreFirstLine());
        if(FileServer.getReadPos() == 0){
            FileServer.setReadPos(FileServer.getStartPos());
        }
    }
    ...
    public void testEnded() {
        ...
        try{
            ...
            LoggerUtil.error("Exception occurred while closing file(s) : {}", e.toString());
        }
    }
    ...
    public void setDelimiter(String delimiter) {
        ...
        if ("\\t".equals(delimiter)) {
            ...
            LoggerUtil.debug("Empty delimiter, ',' (Comma) will be used by default");
            delim = ",";
        }else{
            delim = delimiter;
        }
